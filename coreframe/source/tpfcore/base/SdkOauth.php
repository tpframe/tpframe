...
abstract class SdkOauth{
	 ...
	 * @param array $token 
	 */
	public function __construct($config=[] , $callback="" , $token = null){
		//设置SDK类型
		$class = get_class($this);

		$this->Callback=$callback;		//设置回调地址
     ...
     */
    public static function getInstance($type, $config=[] ,$callback="" , $token = null) {

    	$name = ucfirst(strtolower($type)) . 'SDK';
    	
    	if (class_exists("\\tpfcore\\loginsdk\\".$name)) {
    		$class="\\tpfcore\\loginsdk\\".$name;
    		return new $class($config,$callback,$token);
    	} else {
	 ...
	 * 获取当前授权用户的基本信息
	 */
	abstract public function get_user_info();
}