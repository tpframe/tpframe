...
class BaseStringHelper
{
     ...
     */
    public static function truncate($string, $length, $charset='utf-8' ,$suffix = '...', $encoding = null, $asHtml = false)
    {
        if ($asHtml) {
            return static::truncateHtml($string, $length, $suffix, $encoding ?: $charset);
        }
        
        if (mb_strlen($string, $encoding ?: $charset) > $length) {
            return trim(mb_substr($string, 0, $length, $encoding ?: $charset)) . $suffix;
        } else {
            return $string;
        }
    }
     ...
     */
    public static function startsWith($string, $with, $caseSensitive = true,$charset='utf-8')
    {
        ...
        if ($caseSensitive) {
            ...
            return mb_strtolower(mb_substr($string, 0, $bytes, '8bit'),$charset) === mb_strtolower($with, $charset);
        }
    }
     ...
     */
    public static function endsWith($string, $with, $caseSensitive = true,$charset='utf-8')
    {
        if (!$bytes = static::byteLength($with)) {
            return true;
        }
        if ($caseSensitive) {
            ...
            return mb_strtolower(mb_substr($string, -$bytes, mb_strlen($string, '8bit'), '8bit'), $charset) === mb_strtolower($with, $charset);
        }
    }
