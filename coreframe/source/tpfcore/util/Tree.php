...
class Tree {
	...
	public $id='id';				//默认ID标识名
	public $parentid='parentid';	//默认parentid标识名
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                @extract($value);
                $this->parentid == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
    ...
    public function get_tree_array($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                @extract($value);
                $retarray[$value[$this->id]] = $value;
                $retarray[$value[$this->id]]["child"] = $this->get_tree_array($id, '');
            }
        }
        return $retarray;
    }
    ...
    function get_array($array){
        if($array){//如果有子类
            foreach ($array as $key => $value) {
                $this->treearr[] = $value['id'];
                $this->get_array($value['child']); //调用函数，传入参数，继续查询下级
            }
            return $this->treearr;
        }
    }
