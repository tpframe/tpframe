...
class Core{
    ...
    */
	private static function getModel($name,$layer){
		$model=null;
		$common_service='app'.DS.MODULE_COMMON_NAME.DS.LAYER_SERVICE_NAME.DS.$name;
		$common_logic='app'.DS.MODULE_COMMON_NAME.DS.LAYER_LOGIC_NAME.DS.$name;
		$common_model='app'.DS.MODULE_COMMON_NAME.DS.LAYER_MODEL_NAME.DS.$name;

		$module_service='app'.DS.MODULE_NAME.DS.LAYER_SERVICE_NAME.DS.$name;
		$module_logic='app'.DS.MODULE_NAME.DS.LAYER_LOGIC_NAME.DS.$name;
		$module_model='app'.DS.MODULE_NAME.DS.LAYER_MODEL_NAME.DS.$name;
	...
	final static function loadAction($action,$vars = [],$model="",$layer="",$appendSuffix = false){
		...
		$module_logic='app'.DS.MODULE_NAME.DS.LAYER_LOGIC_NAME.DS.$name;
		$module_model='app'.DS.MODULE_NAME.DS.LAYER_MODEL_NAME.DS.$name;
		$module_controller='app'.DS.MODULE_NAME.DS.LAYER_CONTROLLER_NAME.DS.$name;

		if(!$model){
			if($layer){
				switch ($layer){
					case LAYER_CONTROLLER_NAME : $result = class_exists($module_controller)?Loader::action($action, $vars, LAYER_CONTROLLER_NAME, $appendSuffix):Loader::action('common'.DS.$action, $vars, LAYER_CONTROLLER_NAME, $appendSuffix); break;
			        case LAYER_SERVICE_NAME    : $result = class_exists($module_service)?Loader::action($action, $vars, LAYER_SERVICE_NAME, $appendSuffix):Loader::action('common'.DS.$action, $vars, LAYER_SERVICE_NAME, $appendSuffix); break;
			        case LAYER_LOGIC_NAME      : $result = class_exists($module_logic)?Loader::action($action, $vars, LAYER_LOGIC_NAME, $appendSuffix):Loader::action('common'.DS.$action, $vars, LAYER_LOGIC_NAME, $appendSuffix); break;
			        case LAYER_MODEL_NAME      : $result = class_exists($module_model)?Loader::action($action, $vars, LAYER_MODEL_NAME, $appendSuffix):Loader::action('common'.DS.$action, $vars, LAYER_MODEL_NAME, $appendSuffix); break;
			        default                    : break;
				}
			}else{
				if(class_exists($common_logic)){
					$url='common'.DS.$action;
			...
			$module_service='app'.DS.$model.DS.LAYER_SERVICE_NAME.DS.$name;
			$module_logic='app'.DS.$model.DS.LAYER_LOGIC_NAME.DS.$name;
			$module_model='app'.DS.$model.DS.LAYER_MODEL_NAME.DS.$name;
			$module_controller='app'.DS.$model.DS.LAYER_CONTROLLER_NAME.DS.$name;
			$url=$model.DS.$action;
			if($layer){
				switch ($layer){
	 ...
	 */
	final static function get_addon_class($name = '')
	{
